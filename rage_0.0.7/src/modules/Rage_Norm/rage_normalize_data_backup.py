...
#		QT_exp =  [exp(np.log(qt)-np.log(QT_avg)) for qt in QT_vals]
	...
	def rsample(self): 
		...
		## minImpute
			...
			self.D.samples[t[-1]].notes['ratio'] = t[0]*t_min
			#self.D.samples[t[-1]].notes['ratio'] = t[0]*t_min
	...
	def rsample2(self): 
		...
		feature_comps = rage_comps.features(self.rage).get_solution()


		HOUSEKEEPING, r_key = feature_comps.HOUSEKEEPING, feature_comps.r_key 
		TEST=False
		if TEST:
			feature_comps = rage_comps.features(self.rage).get_f_ratios(RSTAT='MED') 
			feature_comps.predict_missing_sample_values(STAT='MED') 
			
			feature_comps = rage_comps.features(self.rage).get_f_ratios(RSTAT='MED') 
			feature_comps.predict_missing_sample_values(STAT='MEAN') 
			
			feature_comps = rage_comps.features(self.rage).get_f_ratios(RSTAT='MEAN') 
			feature_comps.predict_missing_sample_values(STAT='MEAN') 
		...
		feature_comps.impute_sample_totals() 
		p
		sys.exit() 
#		feature_comps.predict_sample_values() 

#		sys.exit() 
 
		featupre_comps.predict_missing_sample_values() 


		

		t_srt = [[1.0,t[1],t[0],t[2]] if t[0] == 0 else [t[1]/float(t[0]),t[1],t[0],t[2]] for t in feature_comps.expected_totals]

		
		t_min =  min([t[1]/t[0] for t in t_srt]) 
		actual_totals = [] 

		w = open(self.args.prefix+'_ds_medRatio_parameters.out','w') 
		w.write('%-40s %20s %20s %20s %20s %30s\n' % ('---','actual_total','inferred_total','multiplier','min_inferred','effective_downsample_total',))
		for t in t_srt:
			...
			self.D.samples[t[-1]].notes['ratio'] = t[0]*t_min
			actual_totals.append((t[0]*t_min,self.D.samples[t[-1]].name))
			w.write('%-40s %20f %20f %20f %20f %20f\n' % (self.D.samples[t[-1]].name,t[1],t[2],t[0],t_min,t[0]*t_min))
		


		actual_totals.sort() 
#		t_arrange = sorted([[t[1]/(t[1]/float(t[0])),t[0],t[1],t[2]] for t in feature_comps.expected_totals])	


		t_arrange = sorted([[1.0,t[0],t[1],t[2]] if t[0] ==0 else   [t[1]/(t[1]/float(t[0])),t[0],t[1],t[2]] for t in feature_comps.expected_totals])	
		MFE_c,MFE_n = str(round(t_arrange[0][0])),self.D.samples[t_arrange[0][-1]].name	
		MAE_c,MAE_n = str(int(actual_totals[0][0])),actual_totals[0][-1]


		self.progress.notate('Minimum Effective Total '+MFE_c+' ( '+MFE_n+' )\n')
		self.progress.notate('Minimum Actual    Total '+MAE_c+' ( '+MAE_n+' )\n')



   		self.progress.start_minor('Preparing Downsampler',len(self.D.samples),False)
