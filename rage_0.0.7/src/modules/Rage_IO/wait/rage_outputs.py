...
class count_file: 
	...
	def write_row_col_data(self,row_members, col_members, data): 
...
class column_stats:
	...
	def write(self,column_key,row_names,format_key):
		...
		if 'suffix' in format_key: out_name = self.options.prefix+'_'+format_key['suffix']+'.out'
		...
		for n in row_names: 
			row_data = [str(x) for x in [n]+[column_key[k][n] for k in column_names]]	
