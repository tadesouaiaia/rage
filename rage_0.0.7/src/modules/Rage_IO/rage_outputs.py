...
class count_file: 
	...
	def write_row_col_data(self,row_members, col_members, data,format_key={}): 
		if 'name' in format_key: 
			...
			w=open(out_name,'w') 
		elif 'suffix' in format_key:
			out_name = self.options.prefix +'.'+format_key['suffix']
			w=open(out_name,'w') 
		else:
			out_name = self.options.prefix+'.'+suffix+'.cnts'
		

		w=open(out_name,'w') 

		format_str = "%-10s %s\n"
		try:			w.write(format_str % ('---'," ".join([s.name for s in col_members])))
		except AttributeError:	w.write(format_str % ('---'," ".join([s for s in col_members])))
		for f,d in zip(row_members,data):
			 
			w.write(format_str % (f.name," ".join([str(x) for x in d])))


	def write_feature_key(self,feature_key,format_key={}):
		if 'name' in format_key: 
			out_name = format_key['name']
			w=open(out_name,'w') 
		elif 'suffix' in format_key:
	...
	#	format_str = "%-10s %s\n"
		...
		for f,V in feature_key.items(): 
			w.write('%s ' % f)
			for v in V:
				w.write('%s ' % " ".join([str(x) for x in v])) 
			w.write('\n') 
...
class column_stats:
	...
	def write(self,column_key,row_names,format_key):
		...
		if 'width'  in format_key: format_s, format_e = '%-40s ','%'+str(format_key['width'])+'s'
		else:			   format_s,format_e = '%-40s ','%10s'
		w = open(out_name,'w') 
		...
		for n in row_names: 
			rd = [column_key[k][n] for k in column_names]
			rd  = [n.name] + [rx if type(rx) == int else round(rx,3) for rx in [column_key[k][n] for k in column_names]]

			#row_data = [str(x) for x in [n.name]+[column_key[k][n] for k in column_names]]	
			row_data = [str(x) for x in rd]
...
class sim_counts: 
	...
	def write_feature_key(self,feature_key,dex_list,format_key={}):
		...
		w.write('%-35s ' % '---')
		for p in dex_list: w.write('%30s ' % p)
		w.write('\n') 	
		for f,V in feature_key.items(): 
			V = feature_key[f]
			if len(V) == 0: continue 
			w.write('%-35s ' % f)
			for p in dex_list:
				if not V[p]: w.write('%30s ' % '0')
				else:	     w.write('%30s ' % str(V[p][0]))
			w.write('\n') 

	def write_sample_key(self,D,format_key={}):
		if 'width'  in format_key: format_s, format_e = '%-'+str(format_key['width'])+'s ','%'+str(format_key['width'])+'s'
		else:			   format_s,format_e = '%-20s ','%15s'
		...
		attributes = [k for k in D.attribute_class.keys() if k[0:4] == 'SIM:'] + [k for k in D.attribute_class.keys() if k[0:4] != 'SIM:']
		column_names = attributes
		string_format = format_s+' '.join([format_e for s in column_names])+'\n'
		w.write(string_format % tuple(['---']+column_names))

		for s in D: 
			row_data = [s.name]+[s.attributes[n] for n in column_names]
			w.write(string_format % tuple(row_data))
...
class eval_output: 
	...
	def write(self,M,F,ext=None):
		...
		if ext != None: w = open(self.options.prefix+'_'+self.options.dist+'_'+v_str+'-'+'eval_dex_brief_sim'+str(ext)+'.txt','w')
		else: w = open(self.options.prefix+'_'+self.options.dist+'_'+v_str+'-'+'eval_dex_brief.txt','w')

		w.write('%-50s %15s %10s %10s %10s %10s %10s %10s %10s\n' % ('---','pred','beta','p_obs','p_rate','e_rate','FC','cs','pv'))
		for i,f in enumerate(F):

			Y,P = M.Y[i], M.out['params'][i]

			p = sorted([pr for pr in P if pr[-1]])[0]
				
			pObs, pRate,eRate,FC,FS =  S.score(p[2],Y) 
			 



			w.write('%-50s %15s %10f %10d %10.3f %10.3f %10.3f %10.2e %10.2e \n' % (f,p[2],p[1],pObs,pRate,eRate,FC,FS,p[0]))
