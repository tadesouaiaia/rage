...
class sim_boxes:
	def __init__(self,options,X): 
		...
		else:						 self.prefix = 'simulation_boxes'
		self.ax = plt.subplot2grid((self.xLen,self.yLen), (self.xLoc,self.yLoc), rowspan = 1, colspan = 3)


	def add_boxes(self,bar_key,box_key,JITTERS=True,colors=COLORS): 


		box_data =  sorted(box_key.items(),reverse=True) 
		opts,vals = [bd[0] for bd in box_data], [bd[1] for bd in box_data] 
		bars = [bar_key[opt] for opt in opts] 
		bar_width, box_width = 0.5,0.5

		box_pos = [2+(x*2) for x in range(len(opts))] 

		for i,opt in enumerate(opts): self.ax.bar(box_pos[i]-1,bar_key[opt],width=bar_width,color=colors[i]) 

		self.bp = self.ax.boxplot(vals,positions=box_pos, widths=box_width, patch_artist=True, showmeans=True,whis=0.7)

		self.ax.set_xticks([bp-0.33 for bp in box_pos]) 
		self.ax.set_xticklabels([str(x) for x in opts],fontweight='bold',fontsize=15) 
		
		self.ax.set_xlim(0,box_pos[-1]+1) 
		self.ax.set_ylim(min([min(v) for v in vals])-1,max(bars)+5) 

		jitters, legend_items = [], {} 


		for i in range(len(opts)): 


			clr,val = COLORS[i],vals[i] 

		        jitters.append([[box_pos[i] + np.random.normal(0,0.02) for m in range(len(vals[i]))],[v*np.random.normal(1,0.005) for v in vals[i]]])
                        self.bp['boxes'][i].set_edgecolor(colors[i])
                        self.bp['boxes'][i].set_linewidth(1)
                        plt.setp(self.bp['medians'][i], color=colors[i], linewidth=3)
                        plt.setp(self.bp['means'][i], marker='h',markersize=9,markerfacecolor=colors[i])
                        plt.setp(self.bp['caps'][(i*2)+1], color=colors[i],linewidth=1)
                        plt.setp(self.bp['caps'][(i*2)], color=colors[i],linewidth=1)
                        plt.setp(self.bp['whiskers'][i*2], color=colors[i],linewidth=1)
                        plt.setp(self.bp['whiskers'][1+(i*2)], color=colors[i],linewidth=1)
                        plt.setp(self.bp['fliers'][i], markerfacecolor=colors[i], markeredgecolor = colors[i], marker='s',markersize=2.0)
                       	legend_items[opts[i]] = Rect((0,0),1,1,fc=colors[i])
		...
		for i,(xJ,yJ) in enumerate(jitters): 
                	plt.scatter(xJ, yJ, c=colors[i], alpha=0.7,s=50,zorder=9)
	...
	def save(self,preds,covars): 


		preds = ",".join(preds) 
		covariates = ",".join(covars) 

		self.ax.set_title('PREDICTOR: '+preds+'  ( Covariates: '+covariates+' )') 
		
		fig_name = self.options.prefix+"_figure.pdf"
...
#					labels.append(S[i].label.id)
	...
	def save2(self,mt,preds,covars,key={}):
