...
#  		seaborn.set(rc={'axes.facecolor':'black', 'figure.facecolor':'lightblue'})
	...
	def add_scatter_trend(self,x,y,clr=None,R=2): #,xlabel=None,ylabel=None,clr=None):
		...
		if R != 2:
			self.ax.text(xMax,yMax,'$R_i=$'+str(round(R,3)),verticalalignment='top',horizontalalignment='right')
	...
	def add_legend(self,labels,colors):
		...
		self.ax_key[(0,self.yLen-1)].legend(items,labs,ncol=1, loc='upper right',   bbox_to_anchor=(1.2,1.0),fontsize=10)
...
#		plt.legend(items,labs)
	...
	def add_pca_pts(self,P,key={}):

		x,y = 0,1
		if 'components' in key: x,y = key['components'] 
		...
		for i,p in enumerate(P):
			...
			self.ax.scatter(p[x],p[y],color=c,s=size)
