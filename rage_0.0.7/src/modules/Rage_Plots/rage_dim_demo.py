...
class dimplot:
	...
	def add_data(self,dim_members,dim_runs,dim_comps=[],dim_titles=[],dim_large=None,NAMES=False,NAMEOUTLIERS=False):
		...
		for dI,(dim_run,SHOWNAMES) in enumerate(drs): 
			...
			pts, axes_labels = dim_run['pts'], dim_run['axes'] 
			x_comp, y_comp = dcs[dI][0], dcs[dI][1]
			plot_title = None
			if dI < len(dim_titles): plot_title = dim_titles[dI] 
			...
			while True:
				...
				if plot_title != None: self.axes[-1].set_title(plot_title)
...
#		if self.options.title:	plt.suptitle(self.options.title,fontsize=20,fontweight='bold')
		...
		elif self.xLen*self.yLen == 9:   leg = self.axes[0].legend(items,labels,handletextpad=-0.1,columnspacing=0.1,ncol= nc ,  fontsize=fs,bbox_to_anchor=(0.10,1.75),loc=LOC)
		elif self.yLen == 2:   leg = self.axes[0].legend(items,labels,handletextpad=-0.1,columnspacing=0.1,ncol= nc ,  fontsize=fs,bbox_to_anchor=(1.1,1.25),loc=LOC)
	...
	def finish(self,outname=None,suptitle=None):

		#plt.subplots_adjust(top=0.825)

		if suptitle != None: 
			plt.suptitle(suptitle,x=0.5,fontsize=23) 

	  	plt.subplots_adjust(left=0.10, bottom=0.05, right=0.90, top=0.80,wspace=0.25,hspace=0.35) 
