...
class Transform:
	...
	def run(self):
		...
		attributes = [c for c in R.args.color] 
		if R.args.marker != None:   attributes.append(R.args.marker) 
		if R.args.size   != None:   attributes.append(R.args.size) 

		if R.args.command == 'condensation': 
			self.condense()  

		elif R.args.command == 'fabrication':
			self.fabricate() 

		
		elif R.args.command == 'dim': 
			...
			if len(attributes) > 0: 
				R.data.filter_samples_by_attributes(covariates=attributes).scale_and_transform() 

			#if len(R.args.color) > 0:

				#if R.args.size: R.data.filter_samples_by_attributes(self.options.color+[self.options.size]).scale_and_transform() #LOG=True) 
				#else:		R.data.filter_samples_by_attributes(self.options.color).scale_and_transform() 
			#	if R.args.size: R.data.filter_samples_by_attributes(covariates=self.options.color+[self.options.size]).scale_and_transform() #LOG=True) 
			#	else:		R.data.filter_samples_by_attributes(covariates=self.options.color).scale_and_transform() 
			R.data.samples.create_plot_labels(self.options) 

			if R.args.condensedcnts:	
