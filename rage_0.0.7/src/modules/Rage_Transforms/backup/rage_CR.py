...
class CR:
        def __init__(self,args,progress,ldca_components=50):
		...
		self.ldca_comps = ldca_components
		self.tsne_comps = 2 
	...
	def run_ldca(self,fit_matrix,transform_matrix=[]):
         	run = LatentDirichletAllocation(self.ldca_comps).fit(fit_matrix)

		if len(transform_matrix) > 0:  fitPts = run.transform(transform_matrix)	
       		else:			       fitPts = run.transform(fit_matrix)	

		axes = ['LDCA'+str(i+1) for i in range(self.ldca_comps)]


		res = {'type': 'ldca', 'axes': axes, 'coefs': None, 'pts': fitPts, 'run': run} 
