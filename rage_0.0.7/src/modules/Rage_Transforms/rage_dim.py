...
from ..Rage_Plots import rage_dim_demo as dplot
...
def mad_outliers(points, thresh=10):
	...
	if med_abs_deviation == 0: med_abs_deviation+=1 
...
class DIM: 
		...
		self.args,self.progress = R.args, R.progress 
		
	        self.D = R.data
	        self.S, self.F = self.D.samples, self.D.features 
                self.X = [[s.cnts[f.idx] for s in self.S] for f in self.F]
	...
	def run_scaling_demo(self, OBS_MIN = 0.15, TEST=False,INFO=None): 
		out_name = self.args.prefix+'_transformsamples_'

		SL,FL = float(len(self.S)), float(len(self.F)) 

		#X_transpose = [[self.X[i][j] for i in range(len(self.F))] for j in range(len(self.S))]			


		X_raw = [[s.cnts[f.idx] for s in self.S] for f in self.F]

 #               X_data = {'raw': [[s.cnts[f.idx] for s in self.S] for f in self.F], 'med': [], 'randt': []} 
                X_data = {'raw': [], 'med': [], 'randt': []} 
                #X_data = {'raw': X_transpose, 'med': [], 'rand': []} 

		
		F_cutoff, F_clean = dd(bool), dd(bool) 	
		DIST_TEST=True
		TSNE_ITER=6000
#		self.progress.start_minor('Filtering Data...')
		try:
			self.color_labels  = [s.label.vals['color'] for s in self.S]
		except KeyError:
			try: 
				self.color_labels = [s.label.vals['marker']  for s in self.S]


 
			except KeyError:
				self.color_labels  = ['k' for s in self.S]
				DIST_TEST=False	


		if len(list(set(self.color_labels))) < 2: DIST_TEST=False 

		if TEST:	c_vals = [s.cnts for s in self.S] 
		else:		TSNE_ITER = 250

		

		X_names = [] #f for f in self.F] 

		for i,f in enumerate(self.F): 

			L, V = [log(x+1,2) for x in X_raw[i]], X_raw[i]
			NZL, NZ = [l for l in L if l > 0] , [v for v in V if v > 0] 



			try: obsR, obsMed, obsMin, obsMean = len(NZ) / SL, np.median(NZ), min(NZ) , np.mean(NZ)
			except ValueError: continue 

		
			X_names.append(f) 
			NZ_TRIM = sorted(NZ)[0:int(len(NZ)/2.0)]+[1]
 
			V_out =  mad_outliers(np.array(L))  
			NZ_out = mad_outliers(np.array(NZL)) 

			if obsR >= OBS_MIN: F_cutoff[f.name] = True 
			if max(NZ_out) < 5 or max(V_out) < 5: F_clean[f.name] = True 

			X_data['raw'].append(V) 
			X_data['med'].append([v if v > 0 else obsMed for v in V]) 
			X_data['randt'].append([v if v > 0 else random.choice(NZ_TRIM) for v in V]) 

		
		for n,Xd in X_data.items():

			if n != 'raw': continue 

			#X_transpose = [[Xd[i][j] for i in range(len(self.F))] for j in range(len(self.S))]			
			X_cutoff = [V for i,V in enumerate(Xd) if F_cutoff[self.F[i].name]]
			X_clean  = [V for i,V in enumerate(Xd) if F_clean[self.F[i].name]]
			#X_log    = [[log(x+1,2) for x in V] for V in Xd]
		
			#X_names = [f for f in self.F] 
			X_cutoff_names =  [f for f in self.F if F_cutoff[f.name]]
			X_clean_names  =  [f  for f in self.F if F_clean[f.name]]

	
			for Xnr,xType,xNames in [(Xd,'Untrimmed',X_names),(X_clean,'outliers-removed',X_clean_names),(X_cutoff,'obs-cutoff',X_cutoff_names)]: 
			#for Xnr,xType,xNames in [(Xd,'Untrimmed',X_names)]:  #(X_clean,'outliers-removed',X_clean_names),(X_cutoff,'obs-cutoff',X_cutoff_names)]: 


				if xType != 'Untrimmed': continue 
				#if xType != 'Untrimmed': continue 

				self.progress.start_minor('Normalizing '+xType+' Data...')


				Xr = [[float(Xnr[i][j]) for i in range(len(Xnr))] for j in range(len(Xnr[0]))]
				#X_log    = [[log(x+1,2) for x in V] for V in Xr]
				#for Xn,xSpace in [(Xr,'raw'),(X_log,'log')]:

				X = np.array(Xr) 
				X_dict = {'Unscaled': X}  	
				X_dict['standard_scaling'] = StandardScaler().fit_transform(X) 
				X_dict['minmax_scaling']   = MinMaxScaler().fit_transform(X)
				X_dict['robust_scaling']   = RobustScaler(quantile_range=(25, 75)).fit_transform(X)
				X_dict['max_abs']            = MaxAbsScaler().fit_transform(X)
				X_dict['L2']                 =  Normalizer().fit_transform(X)
				X_dict['quantile_uniform']   = QuantileTransformer(output_distribution='uniform').fit_transform(X) 
				X_dict['quantile_normal']    = QuantileTransformer(output_distribution='normal').fit_transform(X) 


				for k,Xm in X_dict.items(): 

	

					self.progress.start_minor('Transforming '+xType+' '+k+' Data...')
					X_log    = [[log(x+1.0,2) if x >=0 else 0 for x in V] for V in Xm]
					pstr = out_name+"_".join([n,xType,k]) 
					pname = " ".join([n,xType,k]) 
					dMatrix = np.matrix(Xm)	
					lMatrix = np.matrix(X_log)	
					CENTER = True 
					if CENTER: 
						dMatrix - dMatrix.mean(axis=0)
						lMatrix - lMatrix.mean(axis=0)



					pca_res     = self.run_pca(dMatrix) 
					raw_tsne = self.run_tsne(pca_res['pts'],ITER=TSNE_ITER) 


					coeff_key  = self.write_pca_data(pca_res, self.S, xNames, pstr+'_raw_pca_')
					coeff_key = self.write_pca_data(raw_tsne, self.S, xNames, pstr+'_raw_tsne_')
	
					skip_raw_tsne = self.run_tsne([p[1::] for p in pca_res['pts']],ITER=TSNE_ITER) 
					
					log_pca_res = self.run_pca(lMatrix) 
					log_tsne = self.run_tsne(log_pca_res['pts'],ITER=TSNE_ITER) 
					hi_log_tsne = self.run_tsne(log_pca_res['pts'],ITER=TSNE_ITER,PERPLEX=320) 
					skip_log_tsne = self.run_tsne([p[1::] for p in pca_res['pts']],ITER=TSNE_ITER) 

					self.progress.start_minor('Scoring/Plotting '+xType+' '+k+' Data...')


					plot_name = 'smart_demo-'+pstr+'.pca'

					coeff_key  = self.write_pca_data(log_pca_res, self.S, xNames, pstr+'_log_pca_')
					coeff_key  = self.write_pca_data(log_tsne, self.S, xNames, pstr+'_log_tsne_')
					coeff_key  = self.write_pca_data(hi_log_tsne, self.S, xNames, pstr+'_log_hitsne_')
					...
					if DIST_TEST:
						...
						log_pca_pts = [pts[0:4] for pts in log_pca_res['pts']]
						log_tsne_pts = [pts[0:2] for pts in log_tsne['pts']]


						log_pca_skip = [pts[1:5] for pts in log_pca_res['pts']]
						log_tsne_skip = [pts[0:2] for pts in skip_log_tsne['pts']]

						raw_tsne_pts = [pts[0:2] for pts in raw_tsne['pts']]
						hi_tsne_pts = [pts[0:2] for pts in hi_log_tsne['pts']]
						
						f_name, f_str = 'log_pca_1_4_tsne',pstr+'_log_pca_1_4_tsne'



						PCA_R,PCA_O,TR,TO   = self.calculate_centroid_dists(log_pca_pts,log_tsne_pts,f_name,f_str,PLOT_NAME=plot_name)
						TEST_SCRS = ' LOG_PCA_1_4:   '+str(PCA_R)+'  '+str(PCA_O)+'  LOG_TSNE_FULL:   '+str(TR)+'  '+str(TO)
						
						f_name, f_str = 'log_pca_2_5_tsne_skip',pstr+'_log_pca_2_5_tsne_skip'
						PCA_R,PCA_O,TR,TO   = self.calculate_centroid_dists(log_pca_skip,log_tsne_skip,f_name,f_str,PLOT_NAME=plot_name)
						TEST_SCRS+='\n  LOG_PCA_2_5:   '+str(PCA_R)+'  '+str(PCA_O)+'  LOG_TSNE_SKIP:   '+str(TR)+'  '+str(TO)

						f_name, f_str = 'raw_tsne_1_2_tsne_hilg',pstr+'_raw_tsne_1_2_tsne_hilog'
						PCA_R,PCA_O,TR,TO   = self.calculate_centroid_dists(log_pca_skip,log_tsne_skip,f_name,f_str,PLOT_NAME=plot_name)
						TEST_SCRS+='\nRAW_TSNE_1_2:   '+str(PCA_R)+'  '+str(PCA_O)+'  LOG_TSNE_HILG:   '+str(TR)+'  '+str(TO)


					else:
						TEST_SCRS = None 

					dimplot = dplot.dimplot(self.args,self.progress,3,3)

						

					pca_data =  [pca_res,pca_res,raw_tsne,log_pca_res,log_pca_res,log_tsne,skip_raw_tsne,skip_log_tsne,hi_log_tsne]
					pca_comps = [(0,1),(2,3),(0,1),(0,1),(2,3),(0,1),(0,1),(0,1),(0,1)] 
					pca_titles = [pname+' RAW',None,'raw-tsne',pname+' LOG',None,'log-tsne','skip-raw-tsne','skip-log-tsne','hi-perplex-log-tsne'] 


					dimplot.add_data(self.S,pca_data,dim_comps=pca_comps,dim_titles=pca_titles, NAMES=True).finish(outname=plot_name,suptitle=TEST_SCRS) 
	...
	def calculate_centroid_dists(self,pca_pts,tsne_pts,pname,pstr,PLOT_NAME='PLOT_FILE.png'):


		wz = open('sample_scrs_'+pstr+'.txt','w') 
#		wz.write('%-20s %20s\n' % ('---',pname))
		dists = dd(lambda: dd(list))
		scrs = dd(lambda: dd(int)) 
		pMeans = dd(lambda: dd(list)) 
		tMeans = dd(lambda: dd(list)) 
		for si,s in enumerate(self.S): 
			for idx in range(len(pca_pts[si])): 
				pMeans[self.color_labels[si]][idx].append(pca_pts[si][idx])
			for idx in range(len(tsne_pts[si])): 	
				tMeans[self.color_labels[si]][idx].append(tsne_pts[si][idx])

		for kp in pMeans.keys():
			for kv in pMeans[kp]: pMeans[kp][kv] = np.mean(pMeans[kp][kv]) 
			pMeans[kp] = np.array([v[1] for v in sorted(pMeans[kp].items())])
		for kp in tMeans.keys():
			for kv in tMeans[kp]: tMeans[kp][kv] = np.mean(tMeans[kp][kv]) 
			tMeans[kp] = np.array([v[1] for v in sorted(tMeans[kp].items())])

		for si,s in enumerate(self.S): 
			s_pca = np.array(pca_pts[si]) 
			s_tsne = np.array(tsne_pts[si]) 
			s_color = self.color_labels[si] 
		
			
			pca_dists  =  sorted([(np.linalg.norm(s_pca-pMeans[clr]),clr) for clr in pMeans.keys()])
			tsne_dists =  sorted([(np.linalg.norm(s_tsne-tMeans[clr]),clr) for clr in tMeans.keys()])
		
			pca_dists = [(True if pca_dists[0][1] == s_color else False,s_color,s.label.id)] + [(round(x[0],3),x[1]) for x in pca_dists]
			tsne_dists = [(True if tsne_dists[0][1] == s_color else False,s_color,s.label.id)] + [(round(x[0],3),x[1]) for x in tsne_dists]
			s_data = {'pca': pca_dists, 'tsne': tsne_dists} 

			

			scrs['pca'][pca_dists[0][0]]+=1 
			scrs['tsne'][tsne_dists[0][0]]+=1 

			pca_match = [pca_dists[j][0] for j in range(1,len(pca_dists)) if pca_dists[j][1] == s_color][0]
			pca_diffs = [pca_dists[j][0] for j in range(1,len(pca_dists)) if pca_dists[j][1] != s_color][0]
			tsne_match = [tsne_dists[j][0] for j in range(1,len(tsne_dists)) if tsne_dists[j][1] == s_color][0]
			tsne_diffs = [tsne_dists[j][0] for j in range(1,len(tsne_dists)) if tsne_dists[j][1] != s_color][0]

			dists['pca'][True].append(pca_match) 
			dists['tsne'][True].append(tsne_match) 
			
			dists['pca'][False].append(pca_diffs) 
			dists['tsne'][False].append(tsne_diffs) 

			s.notes[pname] = s_data
			wz.write('%-20s PCA        %60s %10.2f %10.2f ' % (s.name," ".join([str(z) for z in pca_dists[0]]),pca_match,pca_diffs))
			wz.write('      TSNE       %20s %10.2f %10.2f\n' % (str(tsne_dists[0][0]),tsne_match,tsne_diffs))
		



		PCA_OUT  =  round(np.mean(dists['pca'][False]) / np.mean(dists['pca'][True]),2)
		TSNE_OUT  =  round(np.mean(dists['tsne'][False]) / np.mean(dists['tsne'][True]),2)
		PCA_RATE =   round(scrs['pca'][True] / float(scrs['pca'][True]+scrs['pca'][False]),2)
		TSNE_RATE = round(scrs['tsne'][True] / float(scrs['tsne'][True]+scrs['tsne'][False]),2)
		wz.write('%-8s %20s %50s %10.2f %10.2f %10.2f %10.2f\n' % ('---',pname,PLOT_NAME,PCA_RATE,PCA_OUT,TSNE_RATE,TSNE_OUT))
		wz.close() 
		return PCA_RATE,PCA_OUT,TSNE_RATE,TSNE_OUT
	...
	def run_tsne(self,pca_pts=None,req='FULL',comps=5,ITER=5000,PERPLEX=245):

		#tsne = TSNE(n_components=2, verbose=0, perplexity=200, n_iter=250)
		tsne = TSNE(n_components=2, verbose=0, perplexity=PERPLEX, n_iter=ITER)
		ts = tsne.fit_transform(pca_pts) 
		t_axes = ['TS1','TS2'] 


		tsne_res = {'total_var':False,'axes': t_axes,'pts': ts, 'coefs': False,'type': 'tsne'}
		return tsne_res
	...
	def run_pca(self,data_matrix,comps=50,coef_comps=50,req=False): 
	
		run = PCA(n_components = min(comps,data_matrix.shape[1])).fit(data_matrix)
		
		res={'total_var':sum(run.explained_variance_),'axes':['PC'+str(i+1)+'   '+str(round(v*100,1))+'  %var' for i,v in enumerate(run.explained_variance_ratio_)]}
		res['pts'] =  [p[0:comps] for p in run.transform(data_matrix)]
		#if req == 'FULL':  res['coefs'] =[sorted([(c*c,c,f) for (f,c) in enumerate(C)],reverse=True) for C in run.components_[0:coef_comps]]
		res['coefs'] =[sorted([(c*c,c,f) for (f,c) in enumerate(C)],reverse=True) for C in run.components_[0:coef_comps]]
	...
	def write_pca_data(self,pca_run, cols, rows, out_name, SIZE = 10, PROJECTION_SIZE=2 ):
		...
		if 'coefs' in pca_run and pca_run['coefs'] != False: 
					...
					coeff_key[rows[vi].name].append((j,vl))

      	        	w= open(out_name+'coefs.out','w')
