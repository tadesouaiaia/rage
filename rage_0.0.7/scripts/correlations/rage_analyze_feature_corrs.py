...
def scan(cnts_file,options):
    ...
    if len(gene_key.keys()) > 0:
	gk = sorted(gene_key.keys())
    gJ = sorted(genes.keys())
	...
	for j in range(len(gJ)):
            ...
            mJ,oJ,sJ,cvJ,cntsJ = genes[gJ[j]]
	    
	    if gk[i] == gJ[j]: 	continue 
	    ...
	    nJ = gJ[j] 
	    if nJ in gene_key: nT = gene_key[nJ] 
	    else:              nT = 'NA'
	    
            print gk[i],gJ[j],gene_key[gk[i]],nT,"m/o/s/cv",mI,oI,sI,cvI,"|",mJ,oJ,sJ,cvJ,"| R/S",R,S
            print gk[i],gJ[j],gene_key[gk[i]],nT,"Both,A,B,None",len(cBoth),len(iOnly),len(jOnly),len(cNone),'|',R2,S2
