...
class plot:
	...
	def add_bin_gene(self,pheno,pval,samples,attributes,Yi,Yr,bMock=1,key=None):
...
#		ax2.set_yticks([]) 
		...
		if key:
			...
			ax3 = plt.subplot2grid((self.xLen ,self.yLen), (self.xLoc,4), rowspan = bMock, colspan = 1)
...
def read_key(key_file):
	...
	for line in open(key_file): 
		...
		if line[0] == '---':
			...
			valid_opts = ['CELL_KIND', 'DETAIL_LAYER', 'CELL_TYPE_DETAIL', 'EB_DETAIL', 'MEGASIZE', 'MEA'] #  'GW']
...
if __name__ == '__main__':
	...
	for pval,phenotype,sample_names,attributes,raw,resids in b_list: 
		my_plot.add_bin_gene(phenotype,pval,sample_names,attributes,raw,resids,bMock,key) 
		b+=1 
		if b == xl: break 	
	...
	plt.subplots_adjust(left=0.02, bottom=0.06, right=0.9, top=0.95,wspace=0.2,hspace=0.5)
	plt.show() 
