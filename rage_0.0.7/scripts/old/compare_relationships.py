...
class K_Means:
	...
	def create_groups(self,test_num=20):
		...
		for i,centers,labels in zip(range(len(self.run)),k_centers,k_labels):
			...
			if max(sorted(cc(labels).values(),reverse=True)[1::])/(len(labels)/cLen) < 0.20: continue 
...
def distance_analysis(f_data):
	...
	k_dict = {} 
	scaled_samples = [[scaled_vals[i][j] for i in range(len(scaled_vals))] for j in range(len(scaled_vals[0]))]



	kGroupings = K_Means(scaled_samples).create_groups() 
	sys.exit() 
	### USE K-MEANS TO ESTABLISH NEIGHBORS ### 

	dm =  distance_matrix(scaled_samples,scaled_samples)
