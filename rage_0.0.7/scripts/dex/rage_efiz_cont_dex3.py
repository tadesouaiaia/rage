...
def get_split_key(p_srt):

	P_LEN = len(p_srt) 
	if P_LEN < 10: return False,False
	TR = int(len(p_srt)/4.0)
	tLast = p_srt[TR][1] 
	try: 
		while p_srt[TR][1] == tLast: TR+=1
	...
	diffs = len(list(set([ps[1] for ps in p_srt])))


	if diffs < 2: return False,False
	elif diffs == 2: E_LEN = TR+1
	else:
		if P_LEN - TR < TR: E_LEN = TR+1
		else:		    E_LEN = P_LEN - TR 					

	if TR < 5: return False,False
	if P_LEN - E_LEN < 5: return False,False 
...
def run_script(data_file,options):
	...
	for line in open(options.key):
		...
		if line[0] == '---': 
			...
			key_samples.append(s)
			if '|' not in line:
				for i in range(1,len(line)): 
					try: key[headers[i]][s] = float(line[i])
					except ValueError: key[headers[i]][s] = line[i]
	...
	for line in open(data_file): 
		line = line.split() 
		if line[0] == '---': 
			s_match = [(i,s) for i,s in enumerate(line[1::]) if s in key_samples]
			samples = [s[1] for s in s_match] 
			s_idx   = [s[0] for s in s_match] 
			s_grps  = [grp_key[s[1]] for s in s_match]
			groups = list(set(s_grps))
			if len(groups) > 1: groups.append('ALL') 
			...
			valid_cnts= [cnts[i] for i in s_idx] 
			
			for k in key:



				# COUNT FIRST THEN VAL #
				pair_all  = [] 
				for j in range(len(samples)): 

					if key[k][samples[j]] != 'NA':

						s,c,v,sg = samples[j], valid_cnts[j], key[k][samples[j]] , s_grps[j]
				
						if k.split('@')[0] == 'riseV': v *= 1000
						try: 			vi = int(v) 
						except ValueError:      vi = n_key[v] 
						pair_all.append([c,vi,sg,s]) 

				paired_data = sorted([(valid_cnts[j],key[k][samples[j]]) for j in range(len(samples)) if key[k][samples[j]] != 'NA'])

			
				for group in groups:

					
					if group == 'ALL': 	p_srt = sorted(pair_all, key = lambda X: X[1]) 
					else:			p_srt = sorted([p for p in pair_all if p[2] == group],key=lambda X: X[1])
		


					if split_key[k][group] == False: split_key[k][group] = get_split_key(p_srt) 
					TR,E_LEN = split_key[k][group] 

					if TR == False: continue 

					
					pg1,pg2,pg3 = [x[0] for x in p_srt[0:TR]], [x[0] for x in p_srt[TR:E_LEN]] , [x[0] for x in p_srt[E_LEN::]]
					tr1,tr2,tr3 = [x[1] for x in p_srt[0:TR]], [x[1] for x in p_srt[TR:E_LEN]] , [x[1] for x in p_srt[E_LEN::]]

					
					TR_STRING = '<'+str(tr1[-1])+','+str(tr2[0])+'-'+str(tr2[-1])+','+'>'+str(tr3[0])
			
		
					pgs = [pg1,pg2,pg3] 
					means = [round(np.mean(pg),2) for pg in pgs]
					obs   = [round(len([x for x in pg if x > 0]) / len(pg),3) for pg in pgs]

					m1,m2,m3 = means 
					o1,o2,o3 = obs
					l1,l2,l3 = [len(pg) for pg in pgs] 
					TREND = False 
					if m1 > m3: 
						FC = (-1*m1)/(m3+0.01)
						if l2 < 5 or (m1 > m2 and m2 > m3): TREND = True 
						
 
					else: 	    
						FC = (m3)/(m1+0.01) 				
						if l2 < 5 or (m3 > m2 and m2 > m1): TREND = True 
					pv = stats.ttest_ind(pg1,pg3)[1] 
				
					print gene,group,k,TR_STRING,'|',len(pg1),len(pg2),len(pg3),'|',
					print o1,o2,o3,'|',m1,m2,m3,'|',TREND,round(FC,3),pv	
