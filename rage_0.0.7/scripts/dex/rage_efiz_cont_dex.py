	...
	for line in open(data_file): 
		...
		if line[0] == '---': 
			...
			for k in key:
				...
				pA,pB = [p[0] for p in paired_data],[p[1] for p in paired_data]
			
				if 'NO' in pB or 'YES' in pB:
					pB = [1 if x=='YES' else 0 for x in pB] 
					paired_data = [(x[0],1) if x[1] == 'YES' else (x[0],0) for x  in paired_data]
					pA,pB = [p[0] for p in paired_data],[p[1] for p in paired_data]
				
				R,pv = stats.pearsonr(pA,pB)
				S,spv = stats.spearmanr(pA,pB) 


				pzero,pnon = [x[1] for x in paired_data if x[0] == 0] , [x[1] for x in paired_data if x[0] > 0] 


				tzp = stats.ttest_ind(pzero,pnon)[1] 

				p_srt = sorted(paired_data,key=lambda X: X[1])

				if len(p_srt) > 20:
					TR = int(len(p_srt)/3.0)
					pg1,pg2 = [x[0] for x in p_srt[0:TR]], [x[0] for x in p_srt[-1*TR::]]

					hzp = stats.ttest_ind(pg1,pg2)[1]

					

					print gene,k,R,S,pv,spv,'|',tzp,hzp
