...
def run_script(args):
	...
	progress.start_major('Reading Score File') 
	...
	for line in args.scores:
		...
		if line[0] == '---': 
			...
			for i,x in enumerate(line): 
				...
				xf,xi = float(x),int(x)
				if xf == xi: 
					scrs[headers[i]].append(xi) 
				else:
					scrs[headers[i]].append(xf) 


	print ""	
	np.random.seed(10)  
	rvs = stats.norm.rvs(loc=0, scale=30, size=50)

#	print np.mean(rvs) 

#	print stats.ttest_1samp(rvs,0)
	INTEREST = '0.01_diff'	
...
#	print rvs
	...
	keys = ['0.1','0.05','0.01','0.001']

	obs = {'0.05': 461, '0.1': 911, '0.01': 106, '0.001': 16} 
	## 0.05 = 461 
	## 0.1  = 911 
	## 0.01 = 106 
	## 0.001 = 16 
	...
	for k,vals in scrs.items():
		if k.split('_')[-1] == 'diff': continue 


		differences = np.array([obs[k] - v for v in vals])
		t = (np.mean(differences))/(differences.std(ddof=10)/np.sqrt(len(differences)))
		s = np.random.standard_t(len(differences), size=10)

		v_mean = np.mean(differences) 	
		v_std  = np.std(differences)
		gtz,ltz = len([v for v in differences if v>0]), len([v for v in differences if v<=0])

		print 'test value',k,'mean',v_mean, 'stdev',v_std, 'greater than zero',gtz, 'less or eq than zero',ltz, 'leq zero/total',ltz / (gtz+ltz+0.0) 

		#print len(s),'uh',len(differences) 
		#print s,t

		p = np.sum(s<t) / float(len(s))

		#print (2*min(p,1-p)*100)
		#print("There is a {} % probability that the paired samples stem from distributions with the same means.".format(2 * min(p, 1 - p) * 100))
		...
		v_mean = np.mean(vals) 	
		v_std  = np.std(vals)
		gtz,ltz = len([v for v in vals if v>0]), len([v for v in vals if v<0])
		...
		if k.split('_')[-1] == 'diff': 
			continue
			print k,v_mean, v_std
			print gtz,ltz 
			print stats.chisquare([gtz,ltz], f_exp=[0.5,0.5], ddof=0, axis=0)
		else:
			print k,v_mean, v_std
			print gtz,ltz 
 
			print stats.ttest_1samp(vals,425)
			print stats.ttest_ind(vals,[425,426])
	progress.end() 
...
if __name__ == '__main__':
        ...
        fc = argparse.RawTextHelpFormatter
      	...
      	parser.add_argument('--scores',type=argparse.FileType('r'),default=None,required=True,help='Counts File (Required)')
