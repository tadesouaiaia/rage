	...
	for line in stream:
		...
		if line[0] == '---': continue 
			...
			if lt == "M-stat": 	
				...
				if line[3] == 'obs': md = line[4::]
				elif line[4] == 'obs': md = line[5::] 
				else: 
					print 'wtf'
					sys.exit() 

				obs_vals = [float(x) for x in md[0].split(',')[1:-1]][-1::-1] 
				obs_names = [x for x in md[1].split(',')[1:-1]][-1::-1]
				avg_vals = [float(x) for x in md[3].split(',')[1:-1]][-1::-1]
				avg_names = [x for x in md[4].split(',')[1:-1]][-1::-1] 

				fc1,fc2 = float(md[6]),float(md[7])
				chi1,chi2 = float(md[9]),float(md[10])

				for i,(n,v) in enumerate(zip(obs_names,obs_vals)): 
					iK[f][n]['obs_val'] = v
					iK[f][n]['obs_rnk'] = i+1 

				for i,(n,v) in enumerate(zip(avg_names,avg_vals)): 
					iK[f][n]['avg_val'] = v
					iK[f][n]['avg_rnk'] = i+1 
...
def dex_test(gene,fP,fK,iK,minObs=0.2,minTObs=0.2):


	if min(fP.values()) > 0.1: return 
	kT = [k for k  in fP.keys() if k not in ['EXONIC','TOTAL']] 

	if len(kT) < 2: return 

	if 'EXONIC' not in fP: print 'wtf'


	kOpts = [k for k in iK['EXONIC'] if iK['EXONIC'][k]['obs_val'] > minObs and k not in ['SG','OB']]
	if len(kOpts) < 2: return 

	kStrs = dd(list) 
	kData = dd(lambda: dd(list)) 
	rScr = dd(lambda: dd(list)) 
	vScr = dd(lambda: {}) 
	...
	for k in ['EXONIC']+kT:
		...
		kChiMin,kChiMax = fK[k]['chi']
		kPV  = fP[k]
		oSrt = sorted([(iK[k][t]['obs_val'],t) for t in kOpts],reverse=True)
		vSrt = sorted([(iK[k][t]['avg_val'],t) for t in kOpts],reverse=True)
		oStr = ",".join([os[1] for os in oSrt])
		vStr = ",".join([vs[1] for vs in vSrt])

		if k == 'EXONIC': 
			eStr['obs'] = oStr
			eStr['avg'] = vStr
			eStr['obsmax'] = oSrt[0][1]
			eStr['valmax'] = vSrt[0][1]
			eStr['obs_vals'] = [os[0] for os in oSrt]
			eStr['avg_vals'] = [os[0] for os in vSrt]


		else:


			if oSrt[0][0] < minTObs: continue 	
			if oSrt[0][1] != eStr['obsmax'] and vSrt[0][1] != eStr['valmax']: CAND = True 

			cands.append(kPos) 

			for j,(os,vs) in enumerate(oSrt):	     rScr[vs]['obs'].append(j+1) 
			for j,(ov,vs) in enumerate(vSrt):            rScr[vs]['avg'].append(j+1) 

			kStrs['obs'].append(oStr) 
			kStrs['avg'].append(vStr) 
			kStrs['all'].append(oStr) 
			kStrs['all'].append(vStr) 
			kStrs['obsmax'].append(oSrt[0][1])
			kStrs['valmax'].append(vSrt[0][1])


			rScr[oSrt[0][1]]['obs_ext'].append('MAX') 
			rScr[oSrt[-1][1]]['obs_ext'].append('MIN') 
				
			rScr[vSrt[0][1]]['avg_ext'].append('MAX') 
			rScr[vSrt[-1][1]]['avg_ext'].append('MIN') 
			kPos += 1
		
	if not CAND or len(cands) < 2: return 
	if len(set(kStrs['obs'])) == 1 or len(set(kStrs['avg'])) == 1: return 
	

	kCands = [] 
	for k in kOpts: 
		if len(set(rScr[k]['obs_ext'])) > 1 or  len(set(rScr[k]['avg_ext'])) > 1: kCands.append(k) 
		else:
			if max(rScr[k]['obs'])-min(rScr[k]['obs'])>3 or max(rScr[k]['avg'])-min(rScr[k]['avg'])>3:
				kCands.append(k) 

	if len(kCands)  == 0: return
	if len(kCands) == 1: return 
	...
	if len(kCands) > 1: 
		...
		for i in cands: 
			...
			kChiMin,kChiMax = fK[k]['chi']
			kPV  = fP[k]
			oSrt = sorted([(iK[k][t]['obs_val'],t) for t in kCands],reverse=True)
			vSrt = sorted([(iK[k][t]['avg_val'],t) for t in kCands],reverse=True)
			oStr = ",".join([os[1] for os in oSrt])
			vStr = ",".join([vs[1] for vs in vSrt])

			if oSrt[0][0] < 0.2: continue 

			if oSrt[0][0] == oSrt[-1][0]: continue 
 			if oSrt[0][1] != vSrt[0][1]: continue 

			oD  = oSrt[0][0] - oSrt[-1][0]
			vD  = vSrt[0][0] - vSrt[-1][0]
			oDists.append(oD) 
			vDists.append(vD) 
			

			kCheck.append(i) 
			kTops.append(oSrt[0][1]) 

		if len(list(set(kTops))) < 2: return 
		print ""
		print gene,max(oDists),max(vDists),len(kCheck),fP['EXONIC'],eStr['obs'],eStr['obs_vals'],"|"," ".join(kCands),"GENE"	
		for i in kCheck: 
			k = kT[i] 
			kChiMin,kChiMax = fK[k]['chi']
			kPV  = fP[k]
			...
			print k,oSrt[0],vSrt[0],'|',oSrt[-1],vSrt[-1],fP[k]
