...
class Maximizer:
        #def __init__(self,cnt_key,color_key, options=None):
        def __init__(self,T_universe,trans):

		self.universe = T_universe
		self.uL = range(len(T_universe))
		self.trans    = [t.split(',') for t in trans]
		self.zeros = ",".join(['0' for i in range(len(T_universe))])
		self.data = []


	def add(self,index,ci): 

		
		if sum(ci) == 0: 
			self.data.append(self.zeros) 
			return 

		cp = [round(ic/float(sum(ci)),3) for ic in ci]
		if max(cp) == 1: 
			self.data.append(",".join([str(round(1.0/len(self.trans[cp.index(1)]),2)) if self.universe[i] in self.trans[cp.index(1)] else '0' for i in self.uL]))
			return
		else:
			uni_hist = [dd(float)]
			for p,T in zip(cp,self.trans):
				for t in T: 
					uni_hist[0][t] += p/float(len(T))

			for it in range(5): 
				uni_iter = dd(float) 
				for p,T in zip(cp,self.trans): 
					iprobs = [uni_hist[-1][t] for t in T]
					irel   = [p*(x/sum(iprobs)) if sum(iprobs)>0 else 0 for x in iprobs]
					for rp,t in zip(irel,T):
						uni_iter[t] += rp  
				uni_hist.append({x: y for x,y in uni_iter.items()}) 				
			self.data.append(",".join([str(round(uni_hist[-1][tran],2)) for tran in self.universe]))
...
def parse(ilist):
	...
	for ix in ilist:
		for ix in ilist:
			if len(ix.split('@')) == 2:
				iT.append(ix.split('@')[0]) 
				iE.extend(ix.split('@')[1].split('-')) 
			elif len(ix.split('@')) == 1 and len(ix.split(':'))==2:
				iT.append(ix.split(':')[0])
				iE.extend(ix.split(':')[1].split('-')) 
			else:
				print 'bad',i,ilist	
...
def resolve_isos(stream,fout='iso_percs'):
...
#	w = sys.stdout
	for line in stream:
		...
		try: gene_id,iso = feature.split('=')
		except ValueError:
			gene_id = feature.split('=')[0] 
			iso = "=".join(feature.split('=')[1::]) 

		if gene_id != geneID:
			if geneID != None: 
				transcript_names,transcript_strs = resolve(geneID,isos,iso_cnts)

				w.write('%20s %20s %20s\n' % (geneID, ",".join(transcript_names), "\t".join(transcript_strs)))	


			geneID,isos,iso_cnts = gene_id,[iso],[cnts]

		else:
			isos.append(iso)
			iso_cnts.append(cnts) 
